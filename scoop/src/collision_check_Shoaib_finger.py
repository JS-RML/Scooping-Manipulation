...
def clean_scene():
    global scene_pub, scene, table_pub, display_trajectory_trigger_pub
...
def add_environment():
    global scene_pub, scene, table_pub, display_trajectory_trigger_pub, group_manipulator, pose_Go_stone_set
    REFERENCE_FRAME = '/base'
    p = PlanningScene()
    p.is_diff = True    
    ...
    pose_Bowl = geometry_msgs.msg.PoseStamped()
    ...
    for i in range(len(pose_Go_stone_set)):
        pose_Go_stone = geometry_msgs.msg.PoseStamped()
...
def process_start():
    global gripper_command_publisher, odom_pub, odom_c_pub, display_trajectory_trigger_pub, group_manipulator
    global scene_pub, scene, table_pub, display_trajectory_trigger_pub, group_manipulator, pose_Go_stone_set, fingertip_position, gripper_orientation, Feasible_solution_exist
    odom_pub = rospy.Publisher("/odom", Odometry, queue_size=50)
  
    # diaplay center point
    odom_c_pub = rospy.Publisher("/odom_c", Odometry, queue_size=50)
    display_trajectory_trigger_pub = rospy.Publisher('/display_trigger',String,queue_size=20)
