...
class stonesConfig(Config):
    ...
    NUM_CLASSES = 1 + 2  # Background + stones
...
class stonesDataset(utils.Dataset):
    ...
    def load_stones(self, dataset_dir, subset):
        """Load a subset of the stones dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes. We have only one class to add.
        self.add_class("domino", 1, "1")
        self.add_class("domino", 2, "2")
        ...
        # Add a region attribute flat:head/tail
            flats = [t['region_attributes'] for t in a['regions'].values()]
            for i, p in enumerate(flats):
                ...
                #flat[i] = flats[i]['flat']
            #print(flats)
            class_ids = [int(n['flat']) for n in flats]
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        """
        # If not a stones dataset image, delegate to parent class.
        image_info = self.image_info[image_id]
        if image_info["source"] != "domino":
    ...
    def load_flat(self, image_id):
        ...
        # [height, width, instance_count]
        info = self.image_info[image_id]
        flat = ["" for flat in range(len(info["flats"]))]
        for i, p in enumerate(info["flats"]):
            flat[i] = info["flats"][i]['flat']
        return flat

    def image_reference(self, image_id):
        """Return the path of the image."""
        info = self.image_info[image_id]
        if info["source"] == "domino":
            return info["path"]
        else:
...
def train(model):
    ...
    dataset_train.load_stones(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = stonesDataset()
    dataset_val.load_stones(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect stoness.')
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash'")
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/stones/dataset/",
                        help='Directory of the stones dataset')
