...
for l in range(len(cross_section_point_set)):
    ...
    patch = patches.PathPatch(path, facecolor='#7791C2', edgecolor='black', lw=0, zorder=0)
    ax.add_patch(patch)
    ...
    if len(bottom_point)==1:
        ...
        antipodal0, = ax.plot([i[0] for i in bottom_point], [i[1] for i in bottom_point], 'red', lw=3, zorder=5) 
